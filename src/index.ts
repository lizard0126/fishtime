...
export const Config: Schema<Config> = Schema.object({
  fishpondRefreshTime: Schema.number().default(3600000).description('鱼塘刷新时间 (毫秒)'),
  ...
  waitingTimeMax: Schema.number().default(30000).description('每次事件触发的最长等待时间[毫秒]'),
  waitingTimeMin: Schema.number().default(10000).description('每次事件触发的最短等待时间[毫秒]'),
  totalEventMax: Schema.number().default(7).description('每轮钓鱼最多触发事件的总数量'),
  totalEventMin: Schema.number().default(3).description('每轮钓鱼最少触发事件的总数量'),
...
最后，不要担心失败。多多体验和研究这游戏的特色和对应攻略吧~
  ...
  ctx.command('开始钓鱼', '开始钓鱼吧').userFields(['id']).action(async ({ session }) => {
    ...
    if (!temp.fishpond.length) {
      ...
      let affirm = await session.prompt(10000)
      if (affirm?.trim() !== '是') {
        return
      }
    }
    ...
    for (let i = 0; i < eventLen; i++) {
      if (!temp.isPlay(session.userId)) break;
  ...
  function getguildData(guild) {
    ...
    // 若不存在 初始化信息
    if (!callList[info]) callList[info] = {
      fishpond: [], // 鱼塘
      lastInitFishPondTime: 0, // 上一次刷新记录
      playUser: {}, // 正在游玩的用户
      // 初始化鱼塘
      initFishpond: function () {
        if (+new Date - this.lastInitFishPondTime > config.fishpondRefreshTime) {
          // 生产鱼苗
          this.fishpond = setSpecifiedQuantityFishData(random(config.totalRandomFishpondMin, config.totalRandomFishpondMax));
          this.lastInitFishPondTime = +new Date();
          return true
        }
        return false
      },
      // 移除对应鱼
      removeFishItem: function (fishName: string) {
      ...
      getNowFishPondInfo: function () {
        if (!this.fishpond.length) return '当前群内鱼塘似乎还没有鱼'
      ...
      getUserFshHistory: async function (userId) {
        ...
        if (!fishName.length) return '你的库存还没有鱼呢，快去钓些吧~'
      ...
      isPlay: function (userId) {
        if (this.playUser[userId]) return true;
        return false
      },
      // 添加游玩用户
  ...
  const eventCommonData = [
    ...
    },
    {
      ...
      fn: function (arr: any, poss: any, select: any) {
        if (select == '3') {
          poss.poss = poss.poss + 3;
      ...
      fn: function (arr, poss, select) {
        if (select == '3') {
          if (random(0, 100) < 70) {
            this.closeFollow = true;
            poss.poss = poss.poss + 1;
            const fishMsg = arr.length ? arr.map(item => item.name).slice(0, 4).filter(i => i !== undefined).join('、') : '鱼塘里没有鱼哦...，但是有可爱的僵尸娘嗯！'
  ...
  const propsFn = {
    ...
    checkUsePropsData: async function (userId, isUse: boolean = false) {
      ...
      if (!isUse) {
        ...
        const name = Object.keys(data[userId].use)[0]
        data[userId].use[name].timer = data[userId].use[name].timer - 1

        const buff = data[userId].use[name];

        // 道具使用完成 删除对应道具
        if (data[userId].use[name].timer <= 0) {
          delete data[userId].use[name]
        }
  ...
  async function checkFestivalData() {
    ...
    const info = festiva.festival ? festivaData[festiva?.festival] : false
